...
to minutes for the `shutdown` command (minimum 1 minute, rounded up). The actual
delay in seconds is returned in the response so the caller knows when the action
will take effect.
