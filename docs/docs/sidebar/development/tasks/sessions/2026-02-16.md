...
- `internal/provider/node/host/darwin*.go` — Darwin host provider (7 files) with
  GetHostname, GetUptime, GetOSInfo using gopsutil.
- `internal/provider/node/disk/darwin*.go` — Darwin disk provider (3 files) with
...
- `internal/provider/node/load/darwin*.go` — Darwin load provider (3 files) with
