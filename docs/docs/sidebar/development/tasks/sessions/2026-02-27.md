...
- Added `ConsumerMetrics` type and `GetConsumerStats` to `MetricsProvider`
  interface
...
### Health Status Detail Enhancements

- Added `AgentDetail` and `ConsumerDetail` schemas to health OpenAPI spec
- Added per-agent table to health status (hostname, labels, registered age)
- Added per-consumer table to health status (name, pending, ack_pending,
  redelivered)
- Updated `ConsumerStatsFn` to use `stream.ListConsumers()` for per-consumer
  info instead of just `info.State.Consumers` count
- Updated `AgentStatsFn` to return per-agent details from registry
- Updated CLI `displayStatusHealth` to render agent and consumer tables
- Updated health types with `AgentDetail`, `ConsumerDetail` structs
- Updated all health tests (public + integration) with detailed data assertions
