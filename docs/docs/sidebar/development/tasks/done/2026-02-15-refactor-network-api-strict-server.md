...
- Rewrote `types.go`: replaced PingProvider, DNSProvider, TaskClientManager with
  single `JobClient`
...
- Rewrote `network_ping_post.go`: uses `JobClient.QueryNetworkPingAny`, keeps
  validator for request body, keeps `durationToString` helper
