...
Two non-generated packages are below 100% statement coverage. Add tests for the
...
When `QuerySystemHostname` returns an empty string for the display hostname, the
handler falls back to `workerHostname`. No test currently exercises this path.
...
**Test to add:** Write invalid JSON to the response KV key for a broadcast job,
...
When a worker response has an empty `Hostname` field, it is keyed as `"unknown"`
...
- Generated packages (`gen/`, `mocks/`) and `cmd/` are excluded from this goal —
