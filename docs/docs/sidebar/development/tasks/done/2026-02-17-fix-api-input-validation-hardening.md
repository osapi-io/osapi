...
Harden all API endpoints by validating path parameters, query parameters, and
adding missing 400 responses to OpenAPI specs. The prior validation audit only
covered request body fields via `validate:` struct tags.

## Changes

### OpenAPI specs (400 responses added)

- `internal/api/job/gen/api.yaml` — Added 400 to GET /job, GET /job/{id}, DELETE
  /job/{id}
- `internal/api/system/gen/api.yaml` — Added 400 to GET /system/hostname, GET
  /system/status
...
All handlers now validate path params, query params, and body fields. 0 lint
