...
variable overrides. Users currently have no reference for available
configuration options outside of reading the Go source.
...
- Document the `OSAPI_` env var convention (`AutomaticEnv` + `SetEnvPrefix`)
  with key mapping examples (dots become underscores, e.g., `api.server.port` →
  `OSAPI_API_SERVER_PORT`)
...
- The `--osapi-file` / `-f` flag controls config file path (default
  `osapi.yaml`)
