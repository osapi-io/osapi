...
Align operation names across constants, client, and worker. Foundational phase
...
- `internal/job/client/modify.go`: Fix `Operation: "dns.set"` to `"dns.update"`,
  remove ping methods
...
- Worker processor uses `"ping"` as category switch case (not the full operation
  string), so worker code doesn't need changes for the rename
