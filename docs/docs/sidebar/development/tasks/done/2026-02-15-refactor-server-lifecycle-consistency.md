...
Standardize how the CLI powers all server-type commands (API server, job worker,
...
`WaitGroup`-based graceful shutdown. NATS connection cleanup was added to the
worker command (previously missing). The `ServerManager` interface was replaced
by the shared `Lifecycle` interface.
