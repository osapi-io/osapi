...
`role:web` AND `env:prod` AND `rack:us-east-1a` simultaneously. The admin can
...
Extend consumer creation to loop over `w.appConfig.Job.Worker.Labels` and create
a consumer + goroutine for each `label.{key}.{value}` pattern.
...
Update `BuildQuerySubject` / `BuildModifySubject` calls to parse the target and
build the correct subject. The `publishAndCollect` method already handles
...
- **Stream subjects:** If currently `jobs.query.*` (single token wildcard), must
  widen to `jobs.query.>` (multi-token). This is a one-time migration on stream
  recreation.
...
- **Load-balanced label routing:** `role:web:any` to pick one web server (queue
  group per label)
- **Label wildcards:** `env:*` to target all environments
- **Admin CLI for label management:**
  `osapi admin workers list --label role:web`
- **Dynamic label registration:** Workers can update labels at runtime via NATS
...
- Label-based subscriptions scale linearly with the number of unique labels per
