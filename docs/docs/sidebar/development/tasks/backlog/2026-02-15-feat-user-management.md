...
Add user and group management endpoints. An appliance needs to manage local
system accounts for access control and auditing.
...
- `user.create.execute`, `user.update.execute`, `user.delete.execute`,
  `user.password.execute` (modify)
- `group.list.get`, `group.status.get` (query)
- `group.create.execute`, `group.update.execute`, `group.delete.execute`
  (modify)
