...
Add network interface management endpoints. Currently only DNS and ping are
supported. A real appliance needs IP address assignment, interface up/down, and
routing.
...
- Implementations: `netplan_provider.go` (Ubuntu), `linux_provider.go` (generic)
- Return type: `InterfaceInfo` with name, MAC, IPs, MTU, state, speed, duplex,
  stats (rx/tx bytes)
