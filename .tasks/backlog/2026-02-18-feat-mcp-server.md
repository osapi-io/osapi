...
`job worker`, and `nats server`, reusing the same `JobClient` layer and
`Lifecycle` pattern.
...
Each tool gets an optional `target` parameter (defaults to `_any`).
Broadcast targets (`_all`, labels) return per-worker results.
...
- Tool handlers follow the same pattern as REST handlers in
  `internal/api/system/` — check `job.IsBroadcastTarget()`, call the
  appropriate `JobClient` method, format results.
- stdio transport is the standard for local AI tool integration. The server
  runs as a subprocess spawned by the AI tool.
...
- The NATS embedded server's `Stop()` doesn't need a context (internally
  bounded), so the adapter pattern from `cmd/nats_server_start.go` applies
  only to that server, not here.
