...
Add controlled command execution. Ansible's `command`, `shell`, and
`raw` modules are foundational — they allow running arbitrary commands
when no purpose-built module exists. An appliance API needs this as a
fallback for operations not covered by specific endpoints.
