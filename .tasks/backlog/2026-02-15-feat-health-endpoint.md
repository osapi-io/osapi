...
Add standard health check endpoints for load balancers, orchestrators,
and monitoring systems. A production appliance needs to report its own
health.
...
- `/health` — lightweight, no auth required, returns 200 with
  `{"status": "ok"}`
