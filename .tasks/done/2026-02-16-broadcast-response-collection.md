...
- [x] Create `publishAndCollect` to gather multiple responses within a
      timeout window
- [x] Add `responses` and `worker_states` fields to `JobDetailResponse`
      schema to return per-worker results for broadcast jobs
...
- `publishAndCollect()` method waits full timeout, collects all worker
  responses into `map[string]*job.Response` keyed by hostname
...
- `GetJobStatus` now populates `Hostname` from computed status and
  `Result` from single-worker response
