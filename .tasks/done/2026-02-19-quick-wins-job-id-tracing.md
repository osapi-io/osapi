...
7. Rename `RequestID`/`request_id` → `JobID`/`job_id` in job domain
   types, client, and worker code
8. Update CLI docs to show new output format with job IDs
9. Update all tests for new signatures

## Outcome

Job ID is now surfaced in every API response and CLI output. Users can
see the UUID immediately and use `osapi client job get --job-id <id>`
to inspect job status. Naming is consistent (`job_id` everywhere).
All tests pass, lint clean, docs updated.
